...
def plot_sharpe_ratio_curve(df, sharpe_ratios):
    """
    Plot the Sharpe ratio curve over time.
    
    Args:
        df (pd.DataFrame): Original price data with datetime
        sharpe_ratios (pd.Series): Rolling Sharpe ratios
    """
    ...
    plt.plot(dates, sharpe_ratios, label='Rolling Sharpe Ratio')
    ...
    plot_path = os.path.join(results_dir, 'zscore_sharpe_ratio_curve.png')
...
def plot_performance_metrics(df, metrics, strategy):
    ...
    results_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))), 'backtest', 'results')
    os.makedirs(results_dir, exist_ok=True)
    plot_path = os.path.join(results_dir, 'zscore_performance_metrics.png')
    plt.savefig(plot_path, dpi=300, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    logger.info("Performance metrics plot saved to backtest/results/zscore_performance_metrics.png")
